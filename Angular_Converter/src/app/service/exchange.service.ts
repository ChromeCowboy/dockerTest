...
export class ExchangeService {
	private exchangeUrl = "http://localhost:8080/exchange-rate/from/to/amount/?";

	private findAllListUrl = "http://localhost:8080/exchange-rate/findAll";

	constructor(private http: HttpClient) {}

	getExchangeRate(from: string, to: string, amount: string) {
		return this.http.get<Exchange>(
			this.exchangeUrl + `from=${from}&to=${to}&amount=${amount}`
		);
	}
